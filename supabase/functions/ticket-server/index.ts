...
async function getAuthenticatedUser(request: Request) {
  const authHeader = request.headers.get('x-user-authorization') ?? request.headers.get('Authorization');
...
routes.get('/tickets/search', async (c) => {
  try {
...
routes.delete('/tickets/:id', async (c) => {
  try {
    const user = await getAuthenticatedUser(c.req.raw);
    ...
    console.log('Error deleting ticket:', error?.message ?? error);
    return c.json({ error: `Failed to delete ticket: ${error?.message || 'Unknown error'}` }, 500);
  }
});
...
routes.get('/debug-auth', async (c) => {
  try {
    const authHeader = c.req.header('Authorization');
    const userAuthHeader = c.req.header('x-user-authorization');

    const tokenHeader = userAuthHeader ?? authHeader;
...
app.route('/', routes);
app.route('/ticket-server', routes);
